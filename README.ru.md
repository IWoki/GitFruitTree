...
- **Счётчик под деревом.** Живая строка с числом дней на каждой стадии (🌱 почка / 🍃 лист / 🌸 цветок / 🍑 персик) - считается заново при каждой генерации, не хранится отдельно.
...
ствола (например, свежий экспорт из Illustrator - см. ниже), точки
нужно пересчитать:
...
Одна команда делает всё: убирает скрытые слои (Illustrator и подобные
инструменты сохраняют в SVG вообще все слои, просто помечая невидимые
`display:none` - без очистки на кроне вперемешку окажется и то, что
должно быть спрятано), добавляет служебные метки, под которые
подставляются крона/фон/трава/статистика, расширяет холст под строку
статистики снизу и пересчитывает точки роста через distance transform
(отличает тонкие ветки от толстого ствола, включая место, где ветки
расходятся, а не только нижнюю часть, плюс жёстко отрезает всё ниже
кроны - поверхностные корни такие же тонкие, как ветки).

Безопасно запускать повторно - если метки уже на месте, скрипт просто
пересчитает точки заново, ничего не сломает. Настройки -
`TRUNK_CORE_RADIUS`, `EXTRA_MARGIN`, `Y_MAX_FRACTION` в
`scripts/extract-anchors.py`.

Тот же принцип и для `assets/ground.svg`/`assets/fone-leafs.svg`, если
экспортируете их из того же многослойного файла - у них просто нет
своих служебных меток, поэтому им хватает одной очистки:
...
│
├── scripts/
│   ├── setup-trunk.py           # Одна команда: очистка + метки + точки роста
│   ├── extract-anchors.py       # (вызывается из setup-trunk.py) пересчёт точек
│   ├── clean_layers.py          # Убирает скрытые слои из экспорта Illustrator
...
Это открытый пет-проект, и я рад любой помощи - идеям, багрепортам, а особенно новым ассетам (свой арт ствола, стадий роста, фона, травы). Пул-реквесты и issue welcome.